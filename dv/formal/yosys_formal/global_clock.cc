...
// This pass is intended to straightforwardly and blindly transform all $*ff* cells into
// $FF cells and $check cells into $assert etc cells, it does so without introducing new
// FFs, unlike the versions yosys already provides.
...
struct GlobalClockPass : public Pass {
  ...
  void execute(std::vector<std::string> args, RTLIL::Design *design) override
  {
    ...
    for (auto* cell : cells) {
      // Lower $check into $assert, $assume etc, without checking clocking information.
      ...
      if (ff.has_gclk) continue; // FF cell, already happy
      ...
      if (ff.has_aload) {
        ...
        //                               = aload ? (aload_polarity ? AD : D) : (aload_polarity ? D : AD)
        // This sames a cell since aload_polarity is a constant, so we can skip the equality check.
